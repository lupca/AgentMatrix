...
- [x] **Coordinator trong repo CT tự đọc `.env`/DB khi bí** — không phải bug hệ thống, nhưng cần ghi vào `docs/coordinator-rules.md` + instructions: "mọi dữ liệu AGENTMATRIX phải lấy qua tool control-tower, không truy cập DB/file hệ thống trực tiếp"; về dài hạn cân nhắc chạy coordinator trong thư mục riêng như phần setup.
