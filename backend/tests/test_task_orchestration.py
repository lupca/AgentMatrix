...
def test_resolve_spec_plan_model_setting_wins_over_env(orchestration, db_session):
    db_session.add(Setting(key="spec_plan_model", value="anthropic"))
    db_session.commit()
...
def test_resolve_spec_plan_model_project_override_wins_setting(orchestration, db_session):
    db_session.add(Setting(key="spec_plan_model", value="anthropic"))
    project = Project(
        id="SPEC-MODEL-PROJ",
        name="Spec Model Project",
        autonomy_policy={"spec_plan_model": {"provider": "google", "model": "gemini-2.5-flash"}},
    )
    db_session.add(project)
    db_session.commit()

    resolved = orchestration.resolve_spec_plan_model(project)
    assert resolved["provider"] == "google"
    assert resolved["model"] == "gemini-2.5-flash"
    assert resolved["source"] == "project"


def test_resolve_spec_plan_model_agent_reference_requires_capability(orchestration, db_session):
    db_session.add(
        Agent(id="@coordinator", name="Coordinator", role="coordinator",
              cli="claude", provider="anthropic", model="claude-3-5-sonnet-latest",
              capabilities=["coordinator"])
    )
    db_session.add(
        Agent(id="@plain-executor", name="Plain Executor", role="executor",
              cli="codex", capabilities=[])
    )
    db_session.commit()

    project = Project(
        id="SPEC-MODEL-AGENT-OK", name="Ok",
        autonomy_policy={"spec_plan_model": "@coordinator"},
    )
    db_session.add(project)
    db_session.commit()
    resolved = orchestration.resolve_spec_plan_model(project)
    assert resolved["agent_id"] == "@coordinator"
    assert resolved["provider"] == "anthropic"
    assert resolved["model"] == "claude-3-5-sonnet-latest"

    project_bad = Project(
        id="SPEC-MODEL-AGENT-BAD", name="Bad",
        autonomy_policy={"spec_plan_model": "@plain-executor"},
    )
    db_session.add(project_bad)
    db_session.commit()
...
def test_request_spec_plan_model_supervised_opens_pending_gate(orchestration, db_session):
    task = _task(db_session, "SPEC-GATE-001", mode="supervised")
    result = orchestration.request_spec_plan_model(
        task_id=task.id, actor="@planner", idempotency_key="spec-gate-001"
    )
    assert result.applied is False
    assert result.gate_record.status == "pending"
    assert result.gate_record.gate_type == "spec_plan_model"
    db_session.refresh(task)
    assert task.awaiting_approval is True
    assert task.approval_prompt is not None


def test_request_spec_plan_model_bypass_auto_selects_and_audits(orchestration, db_session):
    db_session.add(Setting(key="autonomy", value="auto"))
    db_session.add(Setting(key="auto_max_risk", value="normal"))
    db_session.add(Setting(key="spec_plan_model", value="anthropic"))
    project = Project(id="SPEC-GATE-BYPASS-PROJ", name="Bypass")
    db_session.add(project)
    db_session.commit()

    task = _task(db_session, "SPEC-GATE-002", mode="supervised")
    task.project = "SPEC-GATE-BYPASS-PROJ"
    db_session.commit()

    result = orchestration.request_spec_plan_model(
        task_id=task.id, actor="@planner", idempotency_key="spec-gate-002"
    )
    assert result.applied is True
    assert result.gate_record.status == "approved"
    assert result.context["model_config"]["provider"] == "anthropic"
    db_session.refresh(task)
    assert task.awaiting_approval is False

    audit = (
        db_session.query(AuditLog)
        .filter(AuditLog.task_id == task.id, AuditLog.action == "auto-approved:spec_plan_model")
        .one()
    )
