...
def test_create_dispatch_stream_and_complete_flow(client, db_session):
    ...
    with patch('app.api.dispatch.run_agent') as actor:
        ...
        dispatched = client.post('/api/dispatch', json={'task_id': task_id, 'agent_id': agent_id, 'timeout_seconds': 60})
    ...
    verdict = _run_command(client, thread_id, f'/verdict {task_id} pass')
    assert verdict == {'action': 'verdict', 'task_id': task_id, 'verdict': 'pass', 'new_status': 'done'}
