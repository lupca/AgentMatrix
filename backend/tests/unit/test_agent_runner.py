...
# ---------------------------------------------------------------------------
# wake_coordinator: decision routing and atomic claim (CTV2-133)
# ---------------------------------------------------------------------------
...
def wake_db(monkeypatch, tmp_path):
    ...
    factory = sessionmaker(bind=engine)
    db = factory()
    db.add(Project(id="wake-project", name="Wake Project", repo_root=str(tmp_path)))
    db.commit()
    db.close()
    monkeypatch.setattr(runner, "SessionLocal", factory)
        ...
        session.messages = list(session.messages or []) + [
            {
                "role": "user",
                "content": message,
                "turn_id": f"wake-{source_event_id}",
            },
            {
                "role": "assistant",
                "content": "Handled decision",
                "turn_id": f"wake-{source_event_id}",
            },
        ]
        session.message_count = len(session.messages)
        session._sa_instance_state.session.commit()

    programmatic_turn.side_effect = persist_turn
    coordinator = MagicMock(run_turn_programmatic=programmatic_turn)
    monkeypatch.setattr(runner, "CoordinatorService", MagicMock(return_value=coordinator))
    yield factory, programmatic_turn
    Base.metadata.drop_all(engine)
    engine.dispose()


def _wake_session(factory, session_id, *, status="active", activity=None):
    db = factory()
    db.add(
        Session(
            id=session_id,
            ...
            status=status,
            messages=[],
            last_activity_at=activity or datetime.now(timezone.utc),
        )
    )
    db.commit()
    db.close()
...
def test_wake_coordinator_routes_two_session_failure_to_origin_once(wake_db):
    factory, programmatic_turn = wake_db
    ...
    db = factory()
    assert db.get(TaskEvent, event_id).claimed_by_session_id == "origin"
    assert db.get(Session, "origin").message_count == 2
    assert db.get(Session, "newer").message_count == 0
    db.close()
    programmatic_turn.assert_awaited_once()
    _, message = programmatic_turn.await_args.args
    assert programmatic_turn.session_ids == ["origin"]
    status = json.loads(message.split("\n", 1)[1])
    assert status["task_id"] == "WAKE-ORIGIN"
    assert status["step"] == "run_failed"
    assert status["result"] == {"exit_code": 1}
    assert status["error"] == "execute failed"
    assert set(status["available_actions"]) == {
        "dispatch",
        "cancel",
        "update_task",
        "verdict",
...
def test_wake_coordinator_falls_back_to_newest_active_global_session(wake_db):
    factory, programmatic_turn = wake_db
    now = datetime.now(timezone.utc)
    _wake_session(factory, "archived-origin", status="archived", activity=now)
    _wake_session(factory, "older", activity=now - timedelta(minutes=1))
    _wake_session(factory, "newest", activity=now)
    event_id = _wake_event(
        factory,
        "WAKE-FALLBACK",
        session_id="archived-origin",
    )

    assert runner.wake_coordinator.fn(event_id) == "completed"

    db = factory()
    assert db.get(TaskEvent, event_id).claimed_by_session_id == "newest"
    db.close()
    assert programmatic_turn.session_ids == ["newest"]


def test_wake_coordinator_parks_when_no_session_is_active(wake_db):
    factory, programmatic_turn = wake_db
    _wake_session(factory, "closed-origin", status="closed")
    event_id = _wake_event(factory, "WAKE-PARK", session_id="closed-origin")

    assert runner.wake_coordinator.fn(event_id) == "parked"

    db = factory()
    assert db.get(TaskEvent, event_id).claimed_by_session_id is None
    db.close()
    programmatic_turn.assert_not_awaited()


def test_wake_coordinator_ignores_info_events_without_a_turn(wake_db):
    factory, programmatic_turn = wake_db
    _wake_session(factory, "active")
    event_id = _wake_event(factory, "WAKE-INFO", kind="info")

    assert runner.wake_coordinator.fn(event_id) == "ignored_info"
...
def test_wake_coordinator_double_delivery_creates_one_turn(wake_db):
    factory, programmatic_turn = wake_db
    _wake_session(factory, "active")
    event_id = _wake_event(factory, "WAKE-ONCE")

    assert runner.wake_coordinator.fn(event_id) == "completed"
    assert runner.wake_coordinator.fn(event_id) == "already_claimed"

    db = factory()
    assert db.get(Session, "active").message_count == 2
    assert db.get(TaskEvent, event_id).claimed_by_session_id == "active"
    db.close()
    programmatic_turn.assert_awaited_once()


def test_wake_coordinator_allows_a_new_failure_after_prior_event_claim(wake_db):
    factory, programmatic_turn = wake_db
    _wake_session(factory, "active")
    first_id = _wake_event(factory, "WAKE-RETRY")

    assert runner.wake_coordinator.fn(first_id) == "completed"
    db = factory()
    second = runner.emit_task_event(
        task_id="WAKE-RETRY",
    ...
    assert runner.wake_coordinator.fn(second_id) == "completed"
    assert programmatic_turn.await_count == 2
    assert [
        call.kwargs["source_event_id"]
        for call in programmatic_turn.await_args_list
    ] == [first_id, second_id]


def test_emit_decision_event_wakes_only_after_persisting(wake_db, monkeypatch):
    factory, _ = wake_db
    db = factory()
    db.add(
        Task(
    ...
    db.commit()
    send = MagicMock()
    monkeypatch.setattr(runner.wake_coordinator, "send", send)

    decision = runner._emit_decision_event(
        db,
        task_id="WAKE-EMIT",
        event_type="run_failed",
        payload={"error": "failed"},
    )
    info = runner.emit_task_event(
        task_id="WAKE-EMIT",
        event_type="progress",
        kind="info",
        payload={"percent": 50},
    ...
    assert db.get(TaskEvent, decision.id).kind == "decision"
    assert db.get(TaskEvent, info.id).kind == "info"
    send.assert_called_once_with(decision.id)
    db.close()
...
def driver_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(runner.wake_coordinator, "send", MagicMock())
