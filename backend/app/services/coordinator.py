...
class CoordinatorService:
    ...
    def get_or_create_global_session(self) -> SessionModel:
        """Resolve the active global inbox for compact task rollups."""
        db_session = (
            self.db.query(SessionModel)
            .filter(
            ...
            .order_by(SessionModel.pinned.desc(), SessionModel.last_activity_at.desc())
            .first()
        )
        if db_session is not None:
            return db_session
    ...
    def record_task_rollup(
        ...
        *,
        ...
        """Upsert one compact task result line in the global session."""
        task = self.db.query(TaskModel).filter(TaskModel.id == task_id).first()
        if task is None:
            raise ValueError(f"Task '{task_id}' not found")
        task_session = self.get_or_create_task_session(task_id)
        if global_session_id:
            global_session = self.db.query(SessionModel).filter(
                SessionModel.id == global_session_id,
                SessionModel.context_level == "global",
            ).first()
            if global_session is None:
                raise ValueError(f"Global session '{global_session_id}' not found")
        else:
            global_session = self.get_or_create_global_session()
        ...
        if error or task.error:
            payload["error"] = error if error is not None else task.error
        message = {
            "id": f"task-rollup-{task.id}",
            "role": "system",
        ...
        existing_index = next(
            (
                ...
                if item.get("kind") == "task_rollup" and item.get("task_id") == task.id
            ),
            None,
        )
